...
class _FormState extends State<_Form> {
  ...
  /// Whether the thing being asked for is a report *of* a period.
  ///
  /// A referral letter has a deadline; a report has a scope. Asking both
  /// questions of every request would clutter the form with one that makes no
  /// sense half the time, so the form asks whichever one fits what is being
  /// requested.
  bool get _wantsPeriod => _category.isIssuedReport;
  ...
  Future<void> _pickPeriod() async {
    ...
    final picked = await showDateRangePicker(
      ...
      // A report covers record that exists, so the window is history: there
      // are no readings to report on from next week.
      firstDate: now.subtract(const Duration(days: 365 * 3)),
      lastDate: now,
      helpText: 'Which period should it cover?',
    );
    if (picked == null || !mounted) return;
  ...
  Future<void> _submit() async {
    ...
    try {
      await DocumentRequestsState.instance.submit(
        ...
        neededBy: _wantsPeriod ? null : _neededBy,
        periodFrom: _wantsPeriod ? _period?.start : null,
        periodTo: _wantsPeriod ? _period?.end : null,
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        // One date question, chosen by what is being asked for: a report is
        // of a period, everything else is wanted by a date.
        _DateRow(
          label: _wantsPeriod
              ? 'Period to cover'
              : 'Needed by (optional)',
          value: _wantsPeriod
              ? (_period == null
                    ? 'Choose the dates the report should cover'
                    : '${DateFormat.yMMMd().format(_period!.start)}'
                          ' – ${DateFormat.yMMMd().format(_period!.end)}')
              : (_neededBy == null
                    ? 'Add a date if it is time-sensitive'
                    : DateFormat.yMMMd().format(_neededBy!)),
          set: _wantsPeriod ? _period != null : _neededBy != null,
          onTap: _wantsPeriod ? _pickPeriod : _pickNeededBy,
          onClear: _wantsPeriod
              ? (_period == null ? null : () => setState(() => _period = null))
              : (_neededBy == null
                    ? null
...
/// One tappable date row, whatever the date means.
///
/// The form asks either for a deadline or for a period depending on what is
/// being requested, and both were the same forty lines of layout — so it is
/// written once and told what it is showing.
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: set
                ? AppColors.brandIndigo.withValues(alpha: 0.35)
                : AppPalette.border(context),
          ),
        ),
        child: Row(
                  ...
                  Text(
                    ...
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: set
                          ...
                          : AppPalette.textMuted(context),
                      fontWeight: set ? FontWeight.w600 : null,
                    ),
                  ),
                ],
              ),
