    ...
    public static function notifyOwner(MedicalDocument $document, string $actorLabel): void
    {
        try {
            AppNotification::create([
                ...
                // Names the document, so tapping the alert opens the file
                // itself rather than dropping the patient at the top of a
                // list to go hunting for what they were just told about.
                'action_arguments' => ['document_id' => (string) $document->id],
                'read' => false,
            ]);
        } catch (\Throwable $e) {
            report($e);
